...
if [ -z "$DATA_PATH" ] || [ -z "$OUTPUT_DIR" ] || [ -z "$NUM_TRAIN_EPOCHS" ] || [ -z "$PER_DEVICE_TRAIN_BATCH_SIZE" ]; then
    echo "错误: 缺少必需参数"
    echo "用法: $0 --data_path <data_path> --output_dir <output_dir> --epochs <num_train_epochs> --batch_size <per_device_train_batch_size> [--swanlab_project <project_name>] [--swanlab_experiment_name <experiment_name>]"
    echo "示例: $0 --data /path/to/data.json --output /path/to/output --epochs 1 --batch_size 4 --project my-project --exp_name experiment-v1"
    exit 1
fi
...
deepspeed llava/train/train_mem_v2.py \
    ...
    --model_name_or_path /hkfs/work/workspace/scratch/lmu_chd4938-MINED_26/null_space_ckpt/sample_ckpt/save_LoRA_Null_adapter_llava_PT_128_ov_pre_64 \
    --version v1 \
    --data_path "$DATA_PATH" \
